...
def test_user_registration(base_url: str):
    ...
    payload = {
        "first_name": "Alice",
        "last_name": "Smith",
        "email": f"alice.smith{uuid4()}@example.com",
        "username": f"alicesmith{uuid4()}",
...
def test_user_login(base_url: str):
    ...
    test_user = {
        "first_name": "Bob",
        "last_name": "Jones",
        "email": f"bob.jones{uuid4()}@example.com",
        "username": f"bobjones{uuid4()}",
        "password": "SecurePass123!",
        "confirm_password": "SecurePass123!"
    }
...
def test_create_calculation_without_auth(base_url: str):
    """Test that creating calculation without authentication fails"""
    url = f"{base_url}/calculations"
    payload = {
        "type": "addition",
        "inputs": [10, 20]
...
def test_list_calculations_without_auth(base_url: str):
    """Test that listing calculations without authentication fails"""
    url = f"{base_url}/calculations"
    response = requests.get(url)
    assert response.status_code == 401, "Expected 401 Unauthorized"

def test_invalid_login_credentials(base_url: str):
...
def test_duplicate_user_registration(base_url: str):
    """Test that registering with duplicate username/email fails"""
    reg_url = f"{base_url}/auth/register"
    ...
    user_data = {
        ...
        "username": f"duplicateuser{uuid4()}",
        "password": "SecurePass123!",
        "confirm_password": "SecurePass123!"
    }
    first_response = requests.post(reg_url, json=user_data)
    assert first_response.status_code == 201, "First registration should succeed"
    
    # Try to register again with same username
    second_response = requests.post(reg_url, json=user_data)
    assert second_response.status_code == 400, "Expected 400 for duplicate registration"

def test_division_by_zero_validation(base_url: str):
    """Test that division by zero is prevented"""
    user_data = {
        "first_name": "Zero",
        "last_name": "Divider",
        "email": f"zerodiv{uuid4()}@example.com",
        "username": f"zerodiv_{uuid4()}",
        "password": "SecurePass123!",
        "confirm_password": "SecurePass123!"
    }
    token_data = register_and_login(base_url, user_data)
    ...
    response = requests.post(url, json=payload, headers=headers)
    assert response.status_code == 422, "Expected 422 for division by zero validation"

def test_insufficient_inputs_validation(base_url: str):
    """Test that calculations with fewer than 2 inputs are rejected"""
    user_data = {
        "first_name": "Single",
        "last_name": "Input",
    ...
    payload = {
        "type": "addition",
        "inputs": [100]  # Only one input
    }
    response = requests.post(url, json=payload, headers=headers)
    assert response.status_code == 422, "Expected 422 for insufficient inputs"